...
import {
  Users, Wrench, FileCheck, DollarSign, CheckCircle2, Clock, Activity, Wallet
} from "lucide-react";
import {
  BarChart, Bar, PieChart, Pie, Cell, ResponsiveContainer, Tooltip, Legend,
  XAxis, YAxis, CartesianGrid, LineChart, Line, ComposedChart
} from "recharts";
...
const CHART_COLORS = {
  ...
  primary: "#6366f1", adminFee: "#8b5cf6", workshopAmount: "#3b82f6", secondary: "#f97316"
};
const TOOLTIP_STYLE = { backgroundColor: "white", borderRadius: "8px", border: "1px solid #e2e8f0", padding: "8px" };
...
const StatusBadge = ({ status }) => {
  ...
  const STYLES = {
    ...
    REJECTED: "bg-red-100 text-red-800", SUCCESS: "bg-green-100 text-green-800",
    FAILED: "bg-red-100 text-red-800"
  };
  
  const ICONS = {
    COMPLETED: <CheckCircle2 className="mr-1 h-3 w-3" />, IN_PROGRESS: <Activity className="mr-1 h-3 w-3" />,
    PENDING: <Clock className="mr-1 h-3 w-3" />, ACCEPTED: <CheckCircle2 className="mr-1 h-3 w-3" />,
    REJECTED: <Clock className="mr-1 h-3 w-3" />, SUCCESS: <CheckCircle2 className="mr-1 h-3 w-3" />,
    FAILED: <Clock className="mr-1 h-3 w-3" />
  };

  return (
    <span className={`px-2 py-1 inline-flex items-center text-xs leading-5 font-semibold rounded-full ${STYLES[statusValue] || "bg-gray-100 text-gray-800"}`}>
  ...
  <Card>
    ...
    {change && (
      <div className="flex items-center text-xs text-gray-500 mt-1">
        <span className={`${parseFloat(change) >= 0 ? "text-emerald-500" : "text-red-500"} font-medium`}>
          {change.startsWith("+") || change.startsWith("-") ? change : `+${change}`}
        </span>
        {change.endsWith("%") ? " from last month" : change.includes("new") ? " new this week" : " from last week"}
      </div>
    )}
    {children}
  </Card>
  ...
  return (
    <Card>
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-500">Latest service requests from users with payment details</p>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead>
            <tr>
              {["User", "Service", "Vehicle", "Workshop", "Status", "Payment"].map(header => (
            ...
            {serviceRequests?.length > 0 ? (
              serviceRequests.map((request, i) => (
                <tr key={`${request.id}-${i}`}>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{request.serviceName}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{request.vehicleType}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{request.workshopName}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm"><StatusBadge status={request.status} /></td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm">
                    {request.payment ? (
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center">
                          <span className="font-medium">{formatCurrency(request.payment.amount)}</span>
                          {request.payment.status && <StatusBadge status={request.payment.status} />}
                        </div>
                        <div className="text-xs text-gray-500">
                          Admin: {formatCurrency(request.payment.platformFee)} | 
                          Workshop: {formatCurrency(request.payment.workshopAmount)}
                        </div>
                      </div>
                    ) : (
      ...
      <div className="mt-4">
        <button 
          className={`w-full py-2 px-4 border border-gray-300 rounded-md text-sm font-medium
            ${hasMore ? 'text-gray-700 hover:bg-gray-50' : 'text-gray-400 cursor-not-allowed'}
            ${loading ? 'bg-gray-100' : ''}`}
...
export function DashboardContent() {
  ...
  const feeBreakdownData = [
    { name: "Platform Fee", value: revenueStats.adminFee?.total || 0, fill: CHART_COLORS.adminFee },
    { name: "Workshop Amount", value: revenueStats.workshopAmount?.total || 0, fill: CHART_COLORS.workshopAmount }
  ];
    ...
    <div className="p-6 space-y-6 bg-gray-50">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <SummaryCard
          ...
          <div className="w-full bg-gray-200 rounded-full h-1.5 mt-3">
            <div className="bg-blue-600 h-1.5 rounded-full" style={{ width: `${Math.min(100, Math.max(10, userStats.growth * 5 + 50))}%` }} />
          </div>
        </SummaryCard>
        ...
        <SummaryCard
          ...
          <div className="mt-3 flex flex-wrap gap-1 text-xs">
            <span className="px-2 py-1 rounded-full bg-purple-50 text-purple-700 flex items-center">
              <Wallet className="mr-1 h-3 w-3" /> Admin Fee: {formatCurrency(revenueStats.adminFee?.total || 0)}
            </span>
            <span className="px-2 py-1 rounded-full bg-amber-50 text-amber-700 flex items-center">
              <Clock className="mr-1 h-3 w-3" /> Pending: {formatCurrency(revenueStats.pending)}
            </span>
          </div>
        </SummaryCard>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {['adminFee', 'workshopAmount'].map((type, i) => (
          <SummaryCard
            key={type}
            title={type === 'adminFee' ? "Admin Fee Revenue" : "Workshop Revenue"}
            value={formatCurrency(revenueStats[type]?.total || 0)}
            change={formatPercentage(type === 'adminFee' ? (revenueStats[type]?.growth || 0) : revenueStats.growth)}
            icon={type === 'adminFee' ? Wallet : Wrench}>
            <div className="mt-3 text-xs">
              <span className="px-2 py-1 rounded-full bg-amber-50 text-amber-700 flex items-center w-fit">
                <Clock className="mr-1 h-3 w-3" /> {formatCurrency(revenueStats[type]?.pending || 0)} pending
              </span>
            </div>
          </SummaryCard>
        ))}
      ...
      {userGrowth?.length > 0 && (
        <ChartCard
          ...
          <LineChart
            ...
            <Tooltip contentStyle={TOOLTIP_STYLE} />
            <Line
              type="monotone" dataKey="count" name="Users"
      ...
      {paymentDistribution?.some(item => item.value > 0) && (
        <ChartCard
          ...
          <ComposedChart
            ...
            />
            <Legend />
            <Bar dataKey="value" name="Total Amount" fill={CHART_COLORS.primary} radius={[4, 4, 0, 0]}>
              {paymentDistribution.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={CHART_COLORS[entry.status] || "#9ca3af"} />
              ))}
            </Bar>
            <Bar dataKey="platformFee" name="Admin Fee" fill={CHART_COLORS.adminFee} radius={[4, 4, 0, 0]} />
            <Bar dataKey="workshopAmount" name="Workshop Amount" fill={CHART_COLORS.workshopAmount} radius={[4, 4, 0, 0]} />
          </ComposedChart>
        </ChartCard>
      )}
